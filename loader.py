...
import const
s_model = SentenceModel(const.DefaultModel)#"shibing624/text2vec-bge-large-chinese")
...
def load_single_file(instance:RequestModel):
    ...
    cur = conn.cursor()
    from psycopg2.extensions import adapt, AsIs
    base_filename = os.path.basename(instance.filename)
    ...
    try:
        ...
        for i, doc in enumerate(chunks, start=1):
            ...
            # Insert the vector data
            cur.execute(f"INSERT INTO {instance.table_name} (path,filename,content,{instance.vector_field}) VALUES (%s,%s,%s,%s);", 
                        (instance.filename, base_filename,doc.page_content, vector_data_adapted))
            conn.commit()
